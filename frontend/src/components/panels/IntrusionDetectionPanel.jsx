...
const IntrusionDetectionPanel = ({ cameraId }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            setLoading(true);
            try {
                // Fetch events specifically for Intrusion Detection on this camera
        ...
        // Optional: Poll every 10 seconds for new intrusion events
            ...
            <div className="glass-panel" style={{ flex: 1, padding: '0', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                <div style={{ padding: '1.5rem', borderBottom: '1px solid var(--panel-border)' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
                        <h3 style={{ margin: 0, color: '#fff', fontSize: '1.1rem' }}>Intrusion Detections</h3>
                        <span style={{ fontSize: '0.8rem', color: '#ef4444', background: 'rgba(239, 68, 68, 0.1)', padding: '0.2rem 0.6rem', borderRadius: '12px' }}>
                            {events.length === 0 ? '● Standby' : '● Security Alerts'}
                        </span>
                    </div>
                    <div style={{ color: 'var(--text-secondary)', fontSize: '0.8rem' }}>
                        Identifying and alerting unauthorized presence in secured zones.
                    </div>
                </div>
                ...
                <div style={{ flex: 1, overflowY: 'auto', padding: '0.5rem' }}>
                    ...
                    {!loading && events.length === 0 && (
                        <div style={{ padding: '2rem', textAlign: 'center', color: 'var(--text-secondary)' }}>
                            No intrusion security events recorded recently.
                        </div>
                    )}
                    ...
                    {!loading && events.map((e, i) => (
                        <div key={i} style={{ padding: '1rem', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.25rem' }}>
                                <span style={{ color: '#ef4444', fontWeight: 'bold', fontSize: '0.9rem' }}>{e.label || 'Unauthorized Entry'}</span>

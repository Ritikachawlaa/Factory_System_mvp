...
const LineCrossingPanel = ({ cameraId }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            setLoading(true);
            try {
                // Fetch events specifically for Line Crossing on this camera
                const [eventsRes, modulesRes] = await Promise.all([
                    httpClient.get('/events', { params: { camera_id: cameraId, module_key: 'line-crossing' } }),
        ...
        // Optional: Poll every 10 seconds for new crossing events
            ...
            <div className="glass-panel" style={{ flex: 1, padding: '0', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                <div style={{ padding: '1.5rem', borderBottom: '1px solid var(--panel-border)' }}>
                    ...
                    <div style={{ color: 'var(--text-secondary)', fontSize: '0.8rem' }}>
                        Real-time tracking of objects crossing virtual tripwires.
                    </div>
                </div>
                ...
                <div style={{ flex: 1, overflowY: 'auto', padding: '0.5rem' }}>
                    ...
                    {!loading && events.length === 0 && (
                        <div style={{ padding: '2rem', textAlign: 'center', color: 'var(--text-secondary)' }}>
                            No line crossing events recorded recently.
                        </div>
                    )}
